@section('title', 'Nouveau Type de matériel')
...
<x-app-layout>
    ...
    <div class="mb-6">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <h1 class="text-2xl font-bold text-gray-900">Nouveau type de matériel</h1>
    ...
    <!-- 
        ...
        On gère ici les infos de base ET le constructeur de schéma JSON 
    -->
    <div x-data="{ 
            name: '{{ old('name') }}',
            categoryId: '{{ old('asset_category_id') }}',
            
            // Notre tableau de lignes (vide par défaut, comme dans la maquette)
            schemaRows:[], 
            
            // Cette fonction nettoie les lignes vides et construit le JSON attendu par le serveur
            generateJsonSchema() {
                let finalSchema = {};
                this.schemaRows.forEach(row => {
                    // On ne garde que les lignes où la clé n'est pas vide
                    if (row.key.trim() !== '') {
                        // On force la clé en minuscules et sans espaces (snake_case) pour la base de données
                        ...
                        // Si c'est un SELECT, on doit extraire les options séparées par des virgules
        ...
        <form action="{{ route('asset-types.store') }}" method="POST" @submit="document.getElementById('hidden_schema').value = generateJsonSchema();">
            @csrf
            
            <!-- LE CHAMP CACHÉ QUI VA RECEVOIR LE VRAI JSON -->
